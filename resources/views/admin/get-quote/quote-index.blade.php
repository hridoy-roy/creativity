...
@section ('page-title', 'Get-Quote || Lists')
@push ('page-css')
<style>
</style>
@endpush
...
<div class="app-title">
  ...
  <ul class="app-breadcrumb breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="#">Get A Quote Lists</a></li>
  </ul>
</div>
<div class="row">
  <div class="col-md-12">
            ...
            <tbody>
              @foreach ($datas as $key=>$data)
                  <tr>
                      ...
                      <td>{{ $data->email}}</td>
                      <td>{{ $data->quantity}}</td>
                      <td>{{ $data->sellection}}</td>
                      <td>{{ $data->created_at }}</td>
                      <td class="d-flex">
                          <a class="btn btn-info btn-sm" href="{{ route('admin.getquote.show', $data->id) }}"><i class="fa fa-eye"></i></a>
                          <button onclick="deleteCategory({{ $data->id }})" type="button" class="btn btn-danger btn-sm waves-effect">
                            </i><i class="fa fa-trash"></i>
                        </button>
                        <form id="delete-form-{{ $data->id }}"
                            style="dispaly:none"
                              action="{{ route('admin.getquote.destroy',$data->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                        </form>
                      </td>
